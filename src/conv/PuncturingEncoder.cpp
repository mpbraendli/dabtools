...
int PuncturingEncoder::process(Buffer* const dataIn, Buffer* dataOut)
{
    ...
    dataOut->resize(d_out_block_size);
    ...
    if (dataIn->size() != d_in_block_size) {
    ...
    for (size_t i = d_out_block_size; i < dataOut->size(); ++i) {
        out[out_count++] = 0;
    }
    assert(out_count == dataOut->size());
    if (out_count != dataOut->size()) {
