...
int tpl_from_stc(struct subchannel_info_t stc)
{
    int TPL;
    if (stc.slForm == 0) {
        TPL = 0x10 | (stc.protlev-1);
    } else {
        TPL = 0x20 | stc.protlev;
    }
    return TPL;
}


SubchannelSource::SubchannelSource(struct subchannel_info_t stc) :
    d_start_address(stc.start_cu),
    d_framesize(stc.bitrate * 3), // TODO divide by 8 ?
    d_protection(tpl_from_stc(stc))
{
    PDEBUG("SubchannelSource::SubchannelSource(...) @ %p\n", this);

    const int TPL = tpl_from_stc(stc);
